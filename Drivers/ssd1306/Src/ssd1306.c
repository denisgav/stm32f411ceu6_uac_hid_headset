...
int32_t ssd1306_UpdateScreen_FSM = -1;

void ssd1306_Reset(void) {
	/* for I2C - do nothing */
	ssd1306_UpdateScreen_FSM = -1;
}

// Send a byte to the command register
HAL_StatusTypeDef ssd1306_WriteCommand(uint8_t byte) {
	return HAL_I2C_Mem_Write(&SSD1306_I2C_PORT, SSD1306_I2C_ADDR, 0x00, 1,
			&byte, 1, HAL_MAX_DELAY);
}

HAL_StatusTypeDef ssd1306_WriteCommand_DMA(uint8_t byte) {
	while (HAL_I2C_GetState(&SSD1306_I2C_PORT) != HAL_I2C_STATE_READY)
		;
	return HAL_I2C_Mem_Write_DMA(&SSD1306_I2C_PORT, SSD1306_I2C_ADDR, 0x00, 1,
			&byte, 1);
}

// Send data
HAL_StatusTypeDef ssd1306_WriteData(uint8_t *buffer, size_t buff_size) {
	return HAL_I2C_Mem_Write(&SSD1306_I2C_PORT, SSD1306_I2C_ADDR, 0x40, 1,
			buffer, buff_size, HAL_MAX_DELAY);
}

HAL_StatusTypeDef ssd1306_WriteData_DMA(uint8_t *buffer, size_t buff_size) {
	while (HAL_I2C_GetState(&SSD1306_I2C_PORT) != HAL_I2C_STATE_READY)
		;
	return HAL_I2C_Mem_Write_DMA(&SSD1306_I2C_PORT, SSD1306_I2C_ADDR, 0x40, 1,
			buffer, buff_size);
}
...
void ssd1306_UpdateScreen_DMA(void) {
	ssd1306_UpdateScreen_FSM = 0;
}

void ssd1306_DMA_task(void) {
	if (ssd1306_UpdateScreen_FSM == -1)
		return;

	if (HAL_I2C_GetState(&SSD1306_I2C_PORT) != HAL_I2C_STATE_READY)
		return;

	// Number of FSM commands is 4*SSD1306_HEIGHT/8
	uint8_t i = ssd1306_UpdateScreen_FSM / 4;
	switch (ssd1306_UpdateScreen_FSM & 0x3) {
	case 0: {
	...
	ssd1306_UpdateScreen_FSM++;
	if(ssd1306_UpdateScreen_FSM >= ((SSD1306_HEIGHT / 8)*4)){
		ssd1306_UpdateScreen_FSM = -1;
	}
}
